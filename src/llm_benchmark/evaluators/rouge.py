...
Supports:
- ROUGE-1, ROUGE-2, ROUGE-L (via rouge-score) for English
- Chinese ROUGE (via rouge-chinese) for Chinese
"""
...
class RougeEvaluator(BaseEvaluator[RougeScores]):
    """Evaluator for ROUGE metrics using rouge-score (English)."""
    ...
    def compute_scores(
        ...
        scores: dict[str, scoring.Score] = self._scorer.score(reference, prediction)
...
class RougeChineseEvaluator(BaseEvaluator[RougeScores]):
    """Evaluator for ROUGE metrics using rouge-chinese (Chinese)."""
    ...
    def compute_scores(
        ...
        # rouge-chinese has no type annotations, so we need to cast
        scores = cast(
            dict[str, dict[str, float]],
            self._chinese_rouge.get_scores(pred_tokens, ref_tokens)[0],
        )
